...
func Julia(f ComplexFunc, n int) image.Image {
	...
	wg2.Add(bounds.Max.X - bounds.Min.X)
	for i := bounds.Min.X; i < bounds.Max.X; i++ {
		go draw_line(i, wg2, bounds, s, f, img)
	}
	wg2.Wait()
	return img
}

func draw_line(i int, wg2 *sync.WaitGroup, bounds image.Rectangle, s float64, f ComplexFunc, img *image.RGBA) {
...
func _main(n int, fn ComplexFunc, wg *sync.WaitGroup) {
